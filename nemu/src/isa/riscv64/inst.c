...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  //RTC测试
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, R(dest) = src1/(unsigned)src2);
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , SB, if(src1 >= (unsigned)src2) s->dnpc = s->pc + offset);
  INSTPAT("??????? ????? ????? 110 ????? 00000 11", lwu    , I, R(dest) = BITS(Mr(src1 + src2, 4),31,0));
